    ...
    if not env_data:
        ...
        **분석 결과:**
        ...
        if selected_school != "전체" and selected_school in env_data:
            ...
            st.markdown(f"### 🎯 {selected_school} 차기 실험 환경 조정 방향성")
            st.markdown("**현재 상태:**")
            st.markdown(f"- 평균 온도: {temp_mean:.1f}°C (변동성: ±{temp_std:.2f})")
            st.markdown(f"- 평균 습도: {humidity_mean:.1f}% (변동성: ±{humidity_std:.2f})")
            st.markdown(f"- 실측 EC: {ec_mean:.2f} (목표: {target_ec})")
            st.markdown("**개선 제안:**")
            st.markdown("1. EC 정밀도: 목표 대비 편차를 ±0.1 이내로 유지")
            st.markdown("2. 온도 안정화: 주야간 온도 변동을 ±2°C 이내로 제어")
            st.markdown("3. 습도 제어: 일정 습도 유지를 위한 자동화 시스템 보완")
        else:
...
with tab3:
    ...
    if not growth_data:
        ...
        st.markdown("### 🎯 주요 발견")
        st.markdown("")
        st.markdown(f"**1. 최적 EC 농도:** EC {optimal_ec} (하늘고)에서 평균 생중량 {max_weight:.2f}g으로 최대값 기록")
        st.markdown("")
        st.markdown("**2. 임계점 분석:**")
        st.markdown("- EC 1.0 (송도고): 대조군 대비 생육 양호")
        st.markdown("- EC 2.0 (하늘고): 최적 성장 구간 ✅")
        st.markdown("- EC 4.0 (아라고): 생중량 감소 시작")
        st.markdown("- EC 8.0 (동산고): 고농도로 인한 생육 저해 확인")
        st.markdown("")
        st.markdown("**3. 환경 안정성의 중요성:**")
        st.markdown("- EC 농도뿐만 아니라 온도/습도 변동성도
